...
class select_stock_product : AppCompatActivity() {
    lateinit var sharedPreferences: SharedPreferences
    lateinit var editor: SharedPreferences.Editor
    val serviceApi = ApiService.create()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_select_stock_product)
        var context = this@select_stock_product
...
//                            Toast.makeText(applicationContext, response.body()!![0].toString(), Toast.LENGTH_LONG).show()
                            ...
                            var adapter = DataSelectProductAdapter(
                                context,
                                response.body()!!,
